...
import { useDeleteProduct, useEditProduct, useRecoverProduct, useSetProductState } from "@/api/products";
...
import { ACTIVE, COLORS, ICONS, INACTIVE, PAGES, PLACEHOLDERS, RECOVER } from "@/common/constants";
import { OnlyPrint, useBreadcrumContext, useNavActionsContext } from "@/components/layout";
...
const ProductPageClient = ({ product }) => {
  ...
  const { data: stockFlows, isLoading: isLoadingsStockFlows } = useGetStockFlow(product.id, {
    ...
    if (product) {
      ...
      const actions = [
        {
          ...
          collapsedTooltip: "Imprimir código de barras",
      ...
      if (!isProductDeleted(product?.state) && !isProductInactive(product?.state)) {
        actions.push({
          ...
          collapsedTooltip: isProductOOS(product?.state) ? "Actualizar stock a en stock" : "Actualizar stock a sin stock",
      ...
      if (!isProductDeleted(product?.state)) {
        ...
        actions.push({
          ...
          text: "Eliminar",
          collapsedTooltip: "Eliminar producto",
          basic: true,
      ...
      if (isProductDeleted(product?.state)) {
        actions.push({
          ...
          text: "Recuperar",
          collapsedTooltip: "Recuperar producto",
