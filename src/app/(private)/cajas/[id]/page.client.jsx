...
import { useCloseCashBalance, useDeleteCashBalance, useEditCashBalance } from "@/api/cashBalances";
...
const CashBalancePageClient = ({ cashBalance }) => {
  ...
  const { mutate: mutateDelete, isPending: isDeletePending } = useMutation({
    mutationFn: () => deleteCashBalance(cashBalance.id),
    onSuccess: (response) => {
      if (response.statusOk) {
  ...
  useEffect(() => {
    ...
    if (RULES.canRemove[role]) {
      ...
      actions.push({
        ...
        color: COLORS.RED,
        text: "Eliminar",
        collapsedTooltip: "Eliminar caja",
        basic: true,
...
export default CashBalancePageClient;
