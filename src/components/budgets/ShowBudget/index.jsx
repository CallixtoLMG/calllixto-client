...
const ShowBudget = ({ budget }) => {
  ...
  const handleDownloadPdf = async () => {
    ...
    const res = await fetch(
      `https://zosudnr9ag.execute-api.sa-east-1.amazonaws.com/pdf/${budget.id}`,
      requestOptions,
    );
    ...
    <>
      <Loader active={isLoading} message="Generando PDF...">
        ...
        <Grid>
                ...
                <Table.Header fullWidth>
                  <ModTableRow>
                    ...
                    {PRODUCTSHEADERS.map((header) => (
                      <ModTableHeaderCell $header key={header.id}>
                        {header.name}
                      </ModTableHeaderCell>
                    ))}
                  </ModTableRow>
          ...
          {budget && (
            <ModButton onClick={handleDownloadPdf} color="blue">
