  ...
  return (
    <ViewContainer>
      ...
      {product?.state === PRODUCT_STATES.DELETED.id &&
        <FieldsContainer>
          <Message negative>
            <p>Este producto se eliminará <b>PERMANENTEMENTE</b> de forma automática el día {threeMonthsDate(product.updateAt)} (90 días desde que se marcó como eliminado).</p>
          </Message>
        </FieldsContainer>}
      ...
      <FieldsContainer>
        {ACCESS.canUpdate[role] &&
          <>
            ...
            <FormField width="20%">
              <Label>Margen (%)</Label>
              <Segment textAlign="right" padding="10px 14px" height="40px" placeholder>
                {margin === "N/A" ? margin : `${margin.toFixed(2)} %`}
              </Segment>
            </FormField>
          </>
        }
