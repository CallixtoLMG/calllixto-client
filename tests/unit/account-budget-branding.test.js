...
test("account budget branding registry contains Maderera and Callixto logos", () => {
  const branding = read("src", "components", "budgets", "PDFfile", "accountBudgetBranding.js");

  assert.match(branding, /const MADERERA_LAS_TAPIAS_BUDGET_LOGO = \{\s*src: "\/accounts\/maderera-las-tapias\.png",\s*alt: "Maderera Las Tapias",\s*width: 160,\s*height: 50,/s);
  assert.match(branding, /showCustomPDFDisclaimer: true/);
  assert.match(branding, /const CALLIXTO_BUDGET_LOGO = \{\s*src: "\/callixto\.png",\s*alt: "Callixto",\s*width: 100,\s*height: 40,/s);
  assert.match(branding, /"maderera-las-tapias": MADERERA_LAS_TAPIAS_BUDGET_LOGO/);
  assert.match(branding, /callixto: CALLIXTO_BUDGET_LOGO/);
});
...
test("account budget logo resolver priority is explicit URL, local registry, then null", () => {
  ...
  assert.doesNotMatch(branding, /CALLIXTO_BUDGET_LOGO\s*;/);
});
